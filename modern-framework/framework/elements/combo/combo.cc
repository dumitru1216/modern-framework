...
bool framework::gui::elements::combo(const std::string& name, std::vector<std::string> values, int* var_name, int max_items)
{
	...
	auto total_size = framework::fonts->sf_pro_bold.measure(values[*var_name]) + framework::fonts->font_awesome_smaller.measure(ICON_FA_CHEVRON_DOWN);
	framework::draw->rect_filled(draw_pos + math_wraper::c_vector_2d(draw_size.x - (total_size.x + 18), -2), math_wraper::c_vector_2d(total_size.x + 18, 22),
		math_wraper::c_color().hex("1D1E25"), 4);

	framework::fonts->sf_pro_bold.draw(draw_pos.x + draw_size.x - (total_size.x + 18) + 6, draw_pos.y + 1,
		values[*var_name], math_wraper::c_color().modulate_normal(200));
	framework::fonts->font_awesome_smaller.draw(draw_pos.x + (draw_size.x - (total_size.x + 18) + 5) + framework::fonts->sf_pro_bold.measure(values[*var_name]).x + 7,
	...
	if (context->focused_id == 0) {
		...
		for (int i = 0; i < values.size(); i++) {
			...
			auto text_size = math_wraper::c_vector_2d(framework::fonts->sf_pro_bold.measure(values[i]).x, 20);
			...
			anim_context_t selected_item = g_anim_base.build(values[i]);
			...
			framework::fonts->sf_pro_bold.draw(text_pos.x, text_pos.y,
				values[i], math_wraper::c_color().modulate_normal(selected_item.m_value));
		}
